...
const lootTable = [
    ...
    {name: 'Light Armour', color: 'lightgrey', ascii: '#', offset: {x: 6, y: 3}},
];

const monsterTable = [
    {name: 'Ogre', color: 'lightgrey', ascii: 'O', offset: {x: 6, y: 3}, health: 6},
    {name: 'Kobold', color: 'green', ascii: 'K', offset: {x: 4, y: 3}, health: 3},
    {name: 'Slime', color: 'darkgreen', ascii: 'S', offset: {x: 3, y: 3}, health: 2},
...
class Spawner {
    ...
    spawnMonster(spawnCount){
        this.spawn(spawnCount, ()=>{
            return new Monster(getRandomInt(this.world.width - 1), getRandomInt(this.world.height - 1), this.world.tileSize, monsterTable[getRandomInt(lootTable.length)]);
        });
    }
